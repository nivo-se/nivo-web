...
export function TemplateDialog({ open, onClose, onSave, template, mode }: TemplateDialogProps) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [prompt, setPrompt] = useState('');
  const [variables, setVariables] = useState<string[]>([]);
  const [newVariable, setNewVariable] = useState('');
  const [scoringDimensions, setScoringDimensions] = useState<PromptTemplate['scoringDimensions']>([]);
  const [newDimension, setNewDimension] = useState({ name: '', description: '', weight: '' });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-12 gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewDimension({ ...newDimension, weight: e.target.value })}
                  className="col-span-2"
                />
                <Button type="button" onClick={handleAddDimension} size="sm" className="col-span-1">
                  <Plus className="w-4 h-4" />
                </Button>
              </div>
            </div>
            ...
            {scoringDimensions.length > 0 && (
              <div className="mt-3 space-y-2">
                {scoringDimensions.map(dim => (
                  <div key={dim.id} className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                    <div className="flex-1">
                      <div className="font-medium text-sm">{dim.name}</div>
                      <div className="text-xs text-gray-500">{dim.description}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline">{(dim.weight * 100).toFixed(0)}%</Badge>
                      <button
                ...
                <div className="flex justify-between items-center text-sm pt-2 border-t">
                  ...
                  <span className={totalWeight === 1.0 ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>
        ...
        <DialogFooter>
          ...
          <Button 
            onClick={handleSave}
            disabled={!name || !description || !prompt || variables.length === 0 || scoringDimensions.length === 0 || Math.abs(totalWeight - 1.0) > 0.01}
          >
