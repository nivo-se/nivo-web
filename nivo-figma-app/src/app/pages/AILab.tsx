...
export default function AILab() {
  ...
  const recentRuns = [...aiRuns].sort((a, b) => 
  ...
  const handleTemplateDuplicate = (template: PromptTemplate) => {
  ...
  const handleTemplateSave = (template: Omit<PromptTemplate, 'id' | 'created_at' | 'created_by'>) => {
    ...
    <div className="h-full overflow-auto bg-gray-50">
      <div className="max-w-5xl mx-auto px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-600">Analyze companies using AI-powered prompt templates</p>
        </div>

        <div className="space-y-4">
          {/* Create New Run Card */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-base font-medium text-gray-900 mb-1">Create New Run</h2>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      if (e.target.value) {
                        navigate(`/ai/run/create?template=${selectedTemplate}&list=${e.target.value}`);
                      }
                    }}
                    defaultValue=""
                  >
                ...
                <p className="text-xs text-gray-500">
                  Estimated cost: ~${(0.20).toFixed(2)} per company
                </p>
              </div>
            ) : (
                ...
                {recentRuns.map(run => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>{run.processed_companies}/{run.total_companies} companies</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-medium text-gray-900">Prompt Templates</h2>
              <span className="text-xs text-gray-500">{promptTemplates.length} templates</span>
            </div>
                          ...
                          <div className="mt-0.5">
                            <ChevronDown 
                              className={`w-4 h-4 text-gray-400 transition-transform ${
                                expandedTemplates.has(template.id) ? 'transform rotate-180' : ''
                              }`}
                            />
                          </div>
                    ...
                    <CollapsibleContent>
                      <div className="px-4 pb-4 pt-0 space-y-3 border-t border-gray-100">
                        <div className="pt-3">
                          {/* Scoring Dimensions */}
                          <div className="mb-3">
                            <p className="text-xs font-medium text-gray-700 mb-2">Scoring Dimensions</p>
                            <div className="flex flex-wrap gap-1.5">
                              {template.scoringDimensions.map(dim => (
                                <span key={dim.id} className="text-xs px-2 py-1 bg-gray-100 text-gray-700 rounded">
                          ...
                          {/* Variables */}
                          <div>
                            <p className="text-xs font-medium text-gray-700 mb-2">Data Fields</p>
                            <div className="flex flex-wrap gap-1">
                              {template.variables.map(variable => (
                                <span key={variable} className="text-xs px-2 py-0.5 bg-gray-50 text-gray-600 rounded font-mono border border-gray-200">
                                  {variable}
                                </span>
                              ))}
                            </div>
